...
To understand what a "log observability platform" is, we first need to define "observability". Observability is the ability to "observe" the inner workings of a software system. This is especially important in the context of distributed applications, where its various functions are spread across different devices, making it difficult to get a holistic picture of the app's inner workings.
...
1. **An agent (shipper)** - Collects the logs and ships them to the ingestor. The shipper can also parse and transform the logs before sending them on.
2. **An ingester** - Receives the logs and inserts them into the storage component.
3. **Log storage** - A database which keeps the increasingly massive stores of logs in long-term storage.
4. **Data visualization** - Queries and displays aggregate data about the logs with the ability to drill down to details.
...
<div className="diagram">[VIDEO DEMO 1: Vector Configuration]</div>
...
### Unilogs Querying and Data Visualization - Grafana
...
The Unilogs platform deployment automatically generates an external IP which the user can use to access Grafana's UI. Once signed in with the user's chosen credentials, they'll see that Unilogs has pre-configured Grafana with a dashboard and provisioned it with the log data from Loki.

See it in action below:

<div className="diagram">[VIDEO DEMO 2: Grafana Interface]</div>
